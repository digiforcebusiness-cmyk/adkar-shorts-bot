...
- Produces:
  ...
  - `CorpusError(RuntimeError)` — the single failure boundary for corpus
    loading. Malformed JSON and wrong-shaped input are re-raised as this,
    never leaked as `JSONDecodeError` or `TypeError`.
